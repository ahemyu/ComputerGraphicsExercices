...
function linearErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, error_scale, metric_func) {
    ...
    for (let i = 0; i < 4 * img.width * img.height; i += 4) {
        ...
        let has_error = loss_r > epsilon || loss_g > epsilon || loss_b > epsilon;
        num_error_pixels += has_error ? 1 : 0;
        // TODO: debug BGR instead of RGB color in linear diff mode for Advanced exercises
...
function errorMap(diff_map_canvas, submission_canvas_id, solution_img_id, points=1.0, error_pixel_threshold=0.05, shader_canvas=true, metric="l1") {
  ...
  if (color == "binary" ) {
    ...
    [num_error_pixels, diff_map_data] = linearErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, error_scale, metric_func);
  }
