...
const functionMapper = {
  Basic1: async args => {
    let canvas = args[0];
    let sceneID = parseInt(args[1]);
    let subtask = args[2]; // "a", "b" or "c", marks which parts of the ray tracer is called
    let pIntersectObjects = (subtask == "b" || subtask == "c"); // true for b) and c)
    let pIndirectLight = (subtask == "c"); // true for c)
    let pMaxRecursionDepth = (args.length > 4) ? parseInt(args[4]) : 1;
    let pNPixels = parseInt(args[3]);
    let pNBRDFSamples = 2;
    let pShowRays = true;
    
    return [canvas, sceneID, pIntersectObjects, pIndirectLight, pMaxRecursionDepth, pNPixels, pNBRDFSamples, pShowRays];
  },
