...
const functionMapper = {
  Basic1: async args => {
    let canvas = args[0];
    let cull_mode = parseInt(args[1]); // 0: None, 1: backface culling, -1: frontface culling
    let depth_mode = parseInt(args[2]); // 0: None, 1: pass less, -1: pass greater
    
    return [canvas, cull_mode, depth_mode];
  },

  showScene: async args => {
    let canvas = args[0];
    let interactive = canvas.id == "scenegraph";
    return [canvas, interactive];
  },

  Basic2: async args => {
    let canvas = args[0];
    let steps = parseInt(args[1]);
    return [canvas, steps];
  },
...
async function get3dCanvasData(x3dElement) {
  return new Promise((resolve) => {
    x3dElement.runtime.exitFrame = () => {
      x3dElement.runtime.exitFrame = null;
      const canvas = x3dElement.querySelector("canvas");
