...
import { mapFunction } from "./functionMapper.js";
 ...
 * 
 *  To add a new mapping, insert a new function-to-args mapping in `functionMapper.js`.
 *  
...
export async function dispatch_call(id, dep_id, container, chain_promise = null) {
  ...
  let mapping_func = mapFunction(name);
  if (mapping_func) {
    args = await Promise.resolve(mapping_func(args))
  }
  if (args.length > 0) {
