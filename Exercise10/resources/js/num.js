...
/** generate nxn identity matrix */
export function id2(n) { return id(2); }
/** generate nxn identity matrix */
export function id3(n) { return id(3); }
/** generate nxn identity matrix */
export function id4(n) { return id(4); }

export function equals(x, y, epsilon = 1e-6) {
    if (!x[0].length) {
...
/** negate self */
export function mneg(x) {
    return negTo(x,x);
}
export function scaTo(y,s,x) {
...
export function polarZ(theta,phi) {
...
export function polarY(theta,phi) {
    let c = Math.cos(phi * torad), s = Math.sin(phi * torad);
    let r = Math.cos(theta * torad);
...
export function transformVector(M,p) {
...
export function affInv(t,u,v,w) {
    //cg.assert(t?.length && u?.length && v?.length && w?.length, "num.aff(): not a vector");
    return new Mat(
        [u[0], v[0], w[0], 0],
        [u[1], v[1], w[1], 0],
...
export function rotateAToB(a,b) {
...
export function matFromSome(t,u,v,w) {
...
export function lookatUVW(eye,at,up) {
    //cg.assert(eye?.length && at?.length && up?.length, "num.lookat(): not a vector");
    let w = dir(eye,at);
...
export class Vec extends Array {
     ...
     * ```
     * 
     * Most function work on a Vec object, but some functions can be used staticly (but need to be imported):
     * 
     * ```js
     ...
     * const v = Num.mul(x, y);   // most functions can also be called without a vector object (if import statement is present)
     * ```
     * 
...
export class Mat extends Array {
     ...
     * ```
     * 
     * Most function work on a Mat object, but some functions can be used staticly (but need to be imported):
     * 
     * ```js
     * x = x.mul(y);                // Multiply x and y
     * x.mul(y);                    // Some functions overwrite the original Mat without needing a reasignment (check the doc)
     * const m = Num.mul(x, y);     // most functions can also be called without a Mat object (if import statement is present)
     * ```
     * 
