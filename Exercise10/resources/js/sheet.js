...
import { dispatch_call } from "./dispatcher.js";

let isChrome = !!window.chrome && !!window.chrome.webstore;
let isFirefox = navigator.userAgent.toLowerCase().includes('firefox');
let advanced_exercises = ["1", "2"];
let contains_advanced_exercises = false; // is set automatically
...
function formatSheet() {
  ...
  contains_advanced_exercises = advanced_exercises.includes(exerciseNr);
  ...
  if (exercisePrefix && title && lecture && exerciseNr && term) {
    ...
    if (contains_advanced_exercises) {
...
function setupSubTasks(scope, number, taskNr, pointsTask) {
  ...
  for (let i in elements) {
    ...
    if (subtask.nodeType == Node.ELEMENT_NODE) {
      ...
      if(typeSubTask == "basic" || contains_advanced_exercises) {
        h3.innerHTML += " Your points: [";
        ...
        h3.innerHTML += " [" + pointsSubTask + "/" + pointsTask + "]";
      }
      ...
      if (subtask.getAttribute("type") == "basic" || contains_advanced_exercises) {
...
function prepareShaders(scope) {
  if (scope === undefined) scope = document;

  let nodes = scope.querySelectorAll('shader');
  let shaders = []
  for (let i in nodes) {
    if (nodes[i].getAttribute) {
      shaders.push(nodes[i])
    }
  }
  for (let i in shaders) {
    ...
    scope.removeChild(shader);
    scope.parentNode.insertBefore(script, scope);

    let load_shaders = scope.getAttribute("shader-toload");
    if (load_shaders == null) {
      load_shaders = shader.getAttribute("id")
    } else {
      load_shaders += ";" + shader.getAttribute("id")
    }
    scope.setAttribute("shader-toload", load_shaders);
  }
}
    ...
    if (node.getAttribute) {
      ...
      if (node.getAttribute("type")) {
        if (node.getAttribute("type") === "eval") {
          ...
          if (student_canvas.nodeName != "IFRAME") { // extra handling for ex01
...
async function executeCalls(ids, promises, container) {
  ...
  for (let i in ids) {
    ...
    dispatch_call(id, -1, container, promise)
  }
}
...
function point_writer() {
  ...
  for (let [task_nr, task_data] of window.eval_point_data) {
    ...
    // TODO would be better to read dynamically
    if (!contains_advanced_exercises || (ex_nr == 1 && task_nr <= 3) || (ex_nr == 2 && task_nr <= 2)) {
      basic_points += task_points;
    } else {
      advanced_points += task_points;
    }
  }
  ...
  if(contains_advanced_exercises) {
    ...
    table_sum.textContent = advanced_points + "/" + max_sum;
  }
...
export function clearCanvas2d(canvas, color=[255,255,255,255]) {
...
export function clearImage(img, color=[255,255,255,255]) {
  let w = img.width;
  let h = img.height;
  for (let x = 0; x < w; x++) {
    for (let y = 0; y < h; y++) {
