...
bool ObjMaterialLibrary::load(const std::string &_file, const std::string &baseFile) {
    ...
    if (!openFile(stream,file,baseFile)) {
        std::cout << "ObjMaterialLoader: Could not open file " << file << std::endl;
        return false;
    }
...
void ObjMaterialLibrary::parseLine(const std::string &line) {
    ...
    } else if (header == "map_Ka") { //textures
...
ObjLoader::ObjLoader(const std::string &file) : file(file) {
    loadFile(file);
}

bool ObjLoader::loadFile(const std::string &_file) {
    this->file = _file;
    ...
    std::cout << "objloader: loading file " << file << std::endl;
...
//    cout << "Triangles: " << faces.size() << endl;
    ...
    std::cout << "Vertices/Faces: " << outVertices.size() << "/" << outTriangles.size() << std::endl;
    return true;
}

void ObjLoader::separateVerticesByGroup() {
...
void ObjLoader::createVertexIndexList() {
    std::vector<bool> vertices_used(vertices.size(),false);

    outVertices.resize(vertices.size());

    for(std::vector<IndexedVertex> &f : faces){
        ObjTriangle fa;
        for(int i=0;i<3;i++) {
            IndexedVertex &currentVertex = f[i];
...
static std::vector<std::vector<IndexedVertex> > triangulateFace(const std::vector<IndexedVertex> &f) {
    std::vector<std::vector<IndexedVertex>> newFaces;

    //more than 3 indices -> triangulate
    std::vector<IndexedVertex> face;
    int cornerCount = 1;
    IndexedVertex startVertex, lastVertex;
    for(const IndexedVertex &currentVertex : f) {
        if (cornerCount <=3 ) {
            if (cornerCount == 1)
...
//v1/vt1/vn1        12/51/1
//v1//vn1           51//4
static IndexedVertex parseIV(std::string &line) {
    IndexedVertex iv;
