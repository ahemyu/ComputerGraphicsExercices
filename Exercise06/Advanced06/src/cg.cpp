...
CG::CG(int w, int h) : Window(w, h) {
    ...
    sphereMesh.load("data/icosphere_highres.obj");
    planeMesh.create_plane_mesh();

    earthColor.load("data/earthColor2.jpg");
    earthBump.load("data/earthBump7.jpg");
    earthClouds.load("data/earthClouds3.jpg");
    earthNight.load("data/earthNight.jpg");
    earthNormal.load("data/earthNormal.jpg");
    earthSpec.load("data/earthSpec.jpg");
    universe.load("data/universe.jpg");

    sun = glm::scale(vec3(10));

    Camera::getCurrent()->lookAt(vec3(28.1349, 1.20666, 3.97974), vec3(27.6059, 1.07783, 3.141), vec3(0, 1, 0));
}
...
void CG::update(float dt) {
    ...
    Camera::getCurrent()->setSpeed(cameraSpeed);
    ...
    mat4 rotateInitial = glm::rotate(glm::radians(23.f), glm::normalize(vec3(0, 0, 1)));
    earth = glm::translate(vec3(30, 0, 0)) * rotateInitial * rotateEarth * glm::scale(vec3(3.5));

    clouds = earth * glm::scale(vec3(1.f + cloudHeight));
}
...
void CG::render() {
    ...
    glUniform1f(5, float(Tesselation));
    ...
    mat4 v = inverse(Camera::getCurrent()->getViewMatrix());
    v = mat4(mat3(v)); // only use rotation part
    mat4 sunTransformation = sun *  v * glm::rotate(glm::radians(-90.f),vec3(1, 0, 0));
...
void CG::renderGui() {
    ...
    ImGui::SliderFloat("cameraSpeed", &cameraSpeed, 0, 0.2f);
    ImGui::Separator();
    ...
    ImGui::SliderFloat("heightScale", &heightScale, 0, 0.1f);
    ImGui::SliderInt("Tesselation Factor", &Tesselation, 1, 20);
    ImGui::Checkbox("useClouds", &useClouds);
    ImGui::SliderFloat("cloudHeight", &cloudHeight, 0, 0.1f);
    ImGui::End();
}
