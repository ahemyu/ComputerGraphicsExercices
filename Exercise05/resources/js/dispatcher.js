...
const functionMapper = {
  Basic1_1: async args => {
    let canvas = args[0];
    let n_samples = parseInt(args[1]);
    let eye_pos = parseVec(args[2]);
    let light_pos = parseVec(args[3]);
    
    return [canvas, n_samples, eye_pos, light_pos];
  },

  Basic1_2: async args => {
    let canvas = args[0];
    let n_segments = parseInt(args[1]);
    let amplitude = parseInt(args[2]);
    let eye_pos = parseVec(args[3]);
    let light_pos = parseVec(args[4]);
    return [canvas, n_segments, amplitude, eye_pos, light_pos];
  },
  ...
  Basic2: async args => {
    let canvas = args[0];
    let start_time = parseFloat(args[1]);
    let do_ambient = args[2] == "true";
    let do_diffuse = args[3] == "true";
    let do_specular = args[4] == "true";
    let n_specular = parseInt(args[5]);

    return [canvas, start_time, do_ambient, do_diffuse, do_specular, n_specular];
  },
