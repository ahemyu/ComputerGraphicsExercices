...
const functionMapper = {
  ...
  Basic1_2: async args => {
    let canvas = args[0];
    let eye = parseVec(args[1]);
    let imagePlane = parseInt(args[2]);
    return [canvas, eye, imagePlane];
  },

  Basic1_3: async args => {
    let canvas = args[0];
    let eye = parseVec(args[1]);
    let fovy_factor = parseInt(args[2]);
    let near = parseInt(args[3]);
    let far = parseInt(args[4]);
    let lookAtPoint = parseVec(args[5]);
    let render_only_canonical_volume = args[6] == "true";
    return [canvas, eye, fovy_factor, near, far, lookAtPoint, render_only_canonical_volume];
  },
  ...
  Basic3: async args => {
    let canvas = args[0];
    let alphas_images_index = parseInt(args[1]);
    return [canvas, alphas_images_index];
  },
