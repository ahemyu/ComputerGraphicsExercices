...
function binaryErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, metric_func) {
    ...
    for (let i = 0; i < 4 * img.width * img.height; i += 4) {
        ...
        let loss_r = metric_func(canvas_data.data[i + 0], img_data.data[i + 0]);
        let loss_g = metric_func(canvas_data.data[i + 1], img_data.data[i + 1]);
        let loss_b = metric_func(canvas_data.data[i + 2], img_data.data[i + 2]);
...
function linearErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, error_scale, metric_func) {
    let num_error_pixels = 0
    for (let i = 0; i < 4 * img.width * img.height; i += 4) {
        // check diff of rgb channels (ignore alpha until there is a reason not to)
        let loss_r = metric_func(canvas_data.data[i + 0], img_data.data[i + 0]);
 ...
 */
function errorMap(diff_map_canvas, submission_canvas_id, solution_img_id, points=1.0, error_pixel_threshold=0.05, shader_canvas=true, metric="l1") {
  ...
  if (submission_canvas_id == "iframe_canvas") {
      img_context.drawImage(img, 0, 0, img.width, img.height);
  } else {
      img_context.drawImage(img, 0, 0);
  }
  
  var submission_canvas = document.getElementById(submission_canvas_id);
  var canvas_data = submission_canvas.getContext('2d').getImageData(0, 0, width, height);
  ...
  if (color == "binary" ) {
    [num_error_pixels, diff_map_data] = binaryErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, metric_func);
  } else {
    // always use the binaryErrorMap for points
    [num_error_pixels, ] = binaryErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, metric_func);
    [, diff_map_data] = linearErrorMap(canvas_data, img, img_data, diff_map_data, epsilon, error_scale, metric_func);
  }
